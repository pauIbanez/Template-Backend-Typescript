...
const startServer = (port: string, app: Application): Promise<void> =>
  new Promise((resolve, reject) => {
    ...
    server.on("error", (error: ServerError) => {
      const messageString =
        error.code === "EADDRINUSE" ? ` Port ${port} in use` : error.message; // Handle the common EADDRINUSE error with a more readable message or leave the default error message
